...
fn main() 
{
    ...
    println!("Enter base: ");
    io::stdin().read_lines(&mut input1).expect("Not a valid string");
    let base:f32 = input1.trim().parse().expect("Not a valid number");

    println!("Enter height: ");
    io::stdin().read_lines(&mut input2).expect("Not a valid string");
    let base:f32 = input2.trim(),parse().expect("Not a a valid number") 
